...
by2olak_data$crawl_date <- format(by2olak_data$crawl_date , "%d-%m-%Y-%H-%M-%S", tz="Etc/GMT-2")
...
Now let's split the rd.path_taken into 2 columns from and to columns
```{r}
...
```{r}
sum(is.na(by2olak_data['rd.header']))
sum(is.na(by2olak_data['rd.start']))
sum(is.na(by2olak_data['rd.destination']))
```
There are NAs in rd.start and rd.destination as some roads have only header. Default values should replace the NAs.
```{r}
...
```{r}
by2olak_data <- subset(by2olak_data, select=c(crawl_date,rd.nm,rd.header,rd.start,rd.destination,rd.ri,
...
```{r , result="hide"}
rename(by2olak_data, c("rd.nm"="road_name", "rd.ri"="road_route_id","rd.stid"="road_status_id",
