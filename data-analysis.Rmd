...
Create a column to describe the day and night in the data. The Night is considered from 12 am to 6 am
...
by2olak_data$time_hours <- as.numeric(by2olak_data$time_hours, units = "hours") - 2
...
```{r}
by2olak_data <- subset(by2olak_data, select=c(day,actual_day,time_hours,day_night,crawl_date,rd.nm,road_header,road_start,road_destination,rd.ri,
...
kable(table(by2olak_data_proper_status$road_status_id, by2olak_data_proper_status$actual_day))
```
...
### 1. Road status id is always updated depending on the last updated previous reports
...
```{r}
table(by2olak_data$road_update_hours_ago)
```
...
```{r}
table(by2olak_data_proper_status$road_update_hours_ago)
```
...
### 2. The Activity of the application is the same during day and night
```{r}
table(by2olak_data$day_night)
pie(table(by2olak_data$day_night))
```

From the pie chart, it is obvious that day time has more activity due to more traffic which could be a result of 
schools and work (rush-hours) unlike night which is more quiet.